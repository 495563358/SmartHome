...
@interface EditAddrViewController ()<AYPlaceViewDelegate,UITextFieldDelegate>
...
- (UIView *)creatUI{
    ...
    _recipients.text = self.info[@"realname"];
    ...
    _mobileF.text = self.info[@"mobile"];
    ...
    _currCity.text = [NSString stringWithFormat:@"%@-%@-%@",_info[@"province"],_info[@"city"],_info[@"area"]];
    ...
    _currStreet.text = _info[@"address"];
...
- (UIView *)creatFootView{
    ...
    [addbtn setTitle:@"保存修改" forState:UIControlStateNormal];
...
-(void)addClick{
    ...
    if (!((_recipients.text)&&( (_mobileF.text.length == 11) || (_mobileF.text.length == 7))&&(_currCity.text)&&(_currStreet.text))) {
    ...
    NSString *selfID = _info[@"id"];
    
    NSDictionary *newDict = @{@"nonce":nonce,@"timestamp":timestamp,@"token":token,@"sign":sign,@"newarea":newarea,@"mobile":mobile,@"address":address,@"realname":realname,@"id":selfID};
    ...
    [[ObjectTools sharedManager] POST:[ResourceFront stringByAppendingString:requestPath] parameters:newDict progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        [_info setValue:realname forKey:@"realname"];
        
        [_info setValue:mobile forKey:@"mobile"];
        
        [_info setValue:address forKey:@"address"];
        
        NSArray *arr = [newarea componentsSeparatedByString:@"-"];
        
        if (arr.count == 3) {
            
            [_info setValue:arr[0] forKey:@"province"];
            
            [_info setValue:arr[1] forKey:@"city"];
            
            [_info setValue:arr[2] forKey:@"area"];
        }
        
        self.editBlock(_info);
        
        
        NSLog(@"%@",[dict[@"result"] objectForKey:@"message"]);
        [self.view makeToast:@"修改地址成功,稍后将为您返回"];

...
- (void)viewDidLoad {
    ...
    _pwdToken.placeholder = @"请重新输入登录密码";
    ...
    UIButton *load = [[UIButton alloc]initWithFrame:CGRectMake((Sc_w-weight)/2, 144+230, weight, 40)];
...
//    load.layer.borderColor= Color_system.CGColor;
    ...
    [load setTitle:@"立即注册" forState:UIControlStateNormal];
//    [load setTitleColor:Color_system forState:UIControlStateNormal];
    ...
    [forget addTarget:self action:@selector(gotoForgetView) forControlEvents:UIControlEventTouchUpInside];
...
-(void)checkClick{
    ...
    NSString *path = @"http://mall.znhomes.com/app/index.php?i=195&c=entry&m=ewei_shopv2&do=mobile&r=account.reg";

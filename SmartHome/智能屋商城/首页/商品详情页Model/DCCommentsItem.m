...
-(instancetype)initWithCommentInfo:(NSDictionary *)commentInfo{
    if (self = [super init]) {
        _comName = commentInfo[@"nickname"];
        _comIcon = commentInfo[@"headimgurl"];
        _comTime = commentInfo[@"createtime"];
        _comContent = commentInfo[@"content"];
        _comSpecifications = @"官配";
        _comReBack = commentInfo[@"reply_content"];
        
        NSString *imageStr = commentInfo[@"images"];
        NSData *data = [imageStr dataUsingEncoding:NSUTF8StringEncoding];
        _imgsArray = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
    }
    return self;
}

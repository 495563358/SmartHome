//
//  AddZigbeeDevice.swift
//  SmartHome
//
//  Created by Smart house on 2017/11/28.
//  Copyright © 2017年 sunzl. All rights reserved.
//

import UIKit

class AddZigbeeDevice: UITableViewController, UIGestureRecognizerDelegate {
    var equip: Equip?
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "fanhui(b)"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(AddZigbeeDevice.handleBack(_:)))
//        navigationItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("保存", comment: ""), style: UIBarButtonItemStyle.plain, target: self, action: #selector(AddZigbeeDevice.handleRightItem(_:)))
        ...
        nextStep.addTarget(self, action: #selector(AddZigbeeDevice.handleRightItem(_:)), for: UIControlEvents.touchUpInside)
        self.view.addSubview(nextStep)
        
        let tap = UITapGestureRecognizer(target: self, action: #selector(AddZigbeeDevice.handleTap(_:)))
        tap.delegate = self
        self.tableView.addGestureRecognizer(tap)
        
        print("-----\(String(describing: self.equip?.hostDeviceCode))===")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 2:
            let cell = tableView.dequeueReusableCell(withIdentifier: "equipnamecell", for: indexPath) as! EquipNameCell
            cell.selectionStyle = UITableViewCellSelectionStyle.none
            cell.leab.text = NSLocalizedString("设备地址:", comment: "")
            cell.complete = {[unowned self](name)in
                print(name)
                self.equip?.equipID = name!
            }
            return cell
        default:
            let cell = tableView.dequeueReusableCell(withIdentifier: "equipconfigcell", for: indexPath) as! EquipConfigCell

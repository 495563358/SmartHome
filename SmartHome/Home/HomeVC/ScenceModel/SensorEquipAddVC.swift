...
class SensorEquipAddVC: UIViewController,UITableViewDelegate,UITableViewDataSource, UIGestureRecognizerDelegate {
    var equip: Equip?
    ...
    var isBox = 1 //判断控制盒还是传感器

    var isget = 0 //判断修改 还是添加 1:修改 0：添加
    
    var PromptText = "" //推送提示文本
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "fanhui(b)"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(SensorEquipAddVC.handleBack(_:)))
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(SensorEquipAddVC.handleTap(_:)))
        tap.delegate = self
        ...
        nextStep.addTarget(self, action: #selector(SensorEquipAddVC.handleRightItem(_:)), for: UIControlEvents.touchUpInside)
    ...
    @objc func handleRightItem(_ barButton: UIBarButtonItem) {
       ...
       // self.equip!.type = "998"
        ...
        if self.isget == 0
        {
            ...
            "modelId":self.equip!.num,//绑定情景模式的ID
            "patternType":"1",
            "pushContent":self.PromptText]
           ...
           // updateSensorsPram
            ...
            "pushContent":self.PromptText]
            print("parameter = \(parameter)")
            BaseHttpService.sendRequestAccess(updateSensorsPram, parameters:parameter as NSDictionary as NSDictionary, success: { (data) -> () in
                //self.equip!.saveEquip()
                print(data)
...
//                return cell
            ...
            case 2:
                let cell = tableView.dequeueReusableCell(withIdentifier: "equipnamecell", for: indexPath) as! EquipNameCell
                cell.leab.text = NSLocalizedString("推送内容:", comment: "")
                if self.equip?.name != ""{
                    cell.equipName.text = self.PromptText
                    
                }
                cell.complete = {[unowned self](name)in
                    print(name as Any)
                    
                    self.PromptText = name!
                }
                cell.selectionStyle =  UITableViewCellSelectionStyle.none
                return cell
            case 3:
                ...
                cell.startTimer.addTarget(self, action: #selector(SensorEquipAddVC.CellTimerAdd(_:)), for: UIControlEvents.touchUpInside)
                self.butArr.append(cell.startTimer)
                
                cell.stopTimer.tag = 1
                cell.stopTimer.addTarget(self, action: #selector(SensorEquipAddVC.CellTimerAdd(_:)), for: UIControlEvents.touchUpInside)

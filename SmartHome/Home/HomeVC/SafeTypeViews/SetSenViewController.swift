...
class SetSenViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = NSLocalizedString("安防设置", comment: "")
        // Do any additional setup after loading the view.
        let but = UIBarButtonItem(image: UIImage(named: "添加房间"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(SetSenViewController.handleBack(_:)))
        
        self.navigationItem.rightBarButtonItem = but
        
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "fanhui(b)"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(SetSenViewController.backClick))
        let swipeGes = UISwipeGestureRecognizer.init(target: self, action: #selector(SetSenViewController.backClick))
    ...
    func analysis(){
        self.modelArr = []
        BaseHttpService.sendRequestAccess(gainSensor, parameters: ["patternType":"1"]) {[unowned self] (data) -> () in
            print(data)
            if data.count == 0{
                return
            }
            for var i in 0...data.count-1
            {
                ...
                self.model = SensorModel.init(deviceCode: dataDict["deviceCode"] as! String, deviceAddress: dataDict["deviceAddress"] as! String, nickName: dataDict["nickName"] as! String, start1: dataDict["No.1"] as! String, start2: dataDict["No.2"] as! String, start3: dataDict["No.3"] as! String, deviceType: dataDict["deviceType"] as! String, securityType: dataDict["securityType"] as! String,sensorstate: dataDict["whetherControlEnclosure"] as! String)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return 200*self.scaleh
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: "SetSenCell")
        if cell == nil {
            cell = SetSenCell.init(style: .default, reuseIdentifier: "SetSenCell")
        }
        (cell as! SetSenCell).model = modelArr[indexPath.row]
        (cell as! SetSenCell).setinit()
        cell!.selectionStyle =  UITableViewCellSelectionStyle.none
        return cell!
    }
    //删除
//    func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
//        let dic = ["deviceCode":modelArr[indexPath.row].sensorhost,"deviceAddress":modelArr[indexPath.row].SensorID]
...
//    }
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        if self.modelArr[indexPath.row].sensorstate == ""{
            //手动添加的传感器
            let rowAction =   UITableViewRowAction(style: UITableViewRowActionStyle.default, title: NSLocalizedString("删除", comment: "")) { [unowned self](action, indexPath) -> Void in
                let dic = ["deviceCode":self.modelArr[indexPath.row].sensorhost,"deviceAddress":self.modelArr[indexPath.row].SensorID,"patternType":"1"]
            ...
            let rowActionSec =   UITableViewRowAction(style: UITableViewRowActionStyle.default, title:  NSLocalizedString("修改", comment: "")) { (action, indexPath) -> Void in
                let sequipAddVC = SensorEquipAddVC()
                let dic = ["deviceCode":self.modelArr[indexPath.row].sensorhost,"deviceAddress":self.modelArr[indexPath.row].SensorID,"patternType":"1"]
                print(dic)
                BaseHttpService.sendRequestAccess(gainAloneSensors, parameters: dic as NSDictionary, success: { (arr) -> () in
                    ...
                    BaseHttpService.sendRequestAccess(getallhost_do, parameters: [:]) {(back) -> () in
                        ...
                        if(back.count <= 0)
                        {
                            ...
                            sequipAddVC.PromptText = arr["pushContent"] as! String

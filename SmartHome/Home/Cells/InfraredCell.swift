...
class InfraredCell: UITableViewCell,UIActionSheetDelegate {
    ...
    override func awakeFromNib() {
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(InfraredCell.longPress(_:)))
    ...
    @IBAction func controlTap(_ sender: AnyObject) {
        ...
        */
        ...
        if self.equip!.type == "501" || self.equip!.type == "502"{
            print("空气净化")
//            if isMoni{
//                let vc = AirConditioningVC()
//                vc.equip = self.equip
...
//            }
            ...
            if isMoni{
                ...
                /* (UIApplication.sharedApplication().delegate as! AppDelegate).infArr = []
                let vc = RedViewController()
                vc.equip = self.equip
                vc.hidesBottomBarWhenPushed=true
                vc.isMoni = self.isMoni
